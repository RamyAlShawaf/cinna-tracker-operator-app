...
public class MobileScannerPlugin: NSObject, FlutterPlugin, FlutterStreamHandler, FlutterTexture, AVCaptureVideoDataOutputSampleBufferDelegate {
    ...
    public func captureOutput(_ output: AVCaptureOutput, didOutput sampleBuffer: CMSampleBuffer, from connection: AVCaptureConnection) {
        ...
        if ((detectionSpeed == DetectionSpeed.normal || detectionSpeed == DetectionSpeed.noDuplicates) && eligibleForScan || detectionSpeed == DetectionSpeed.unrestricted) {
            ...
            DispatchQueue.global(qos: .userInitiated).async { [weak self] in
                ...
                guard let pixelBuffer = self.latestBuffer else { return }
                ...
                do {
                    let barcodeRequest: VNDetectBarcodesRequest = VNDetectBarcodesRequest(completionHandler: { [weak self] (request, error) in
                        ...
                        DispatchQueue.main.async {
                            // Dimensions derived from the pixel buffer; bytes created only if requested
                            let width = CVPixelBufferGetWidth(pixelBuffer)
                            let height = CVPixelBufferGetHeight(pixelBuffer)

                            var bytes: FlutterStandardTypedData? = nil
                            if MobileScannerPlugin.returnImage {
                                var cgImageFromBuffer: CGImage?
                                VTCreateCGImageFromCVPixelBuffer(pixelBuffer, options: nil, imageOut: &cgImageFromBuffer)
                                if let cg = cgImageFromBuffer, let data = cg.jpegData(compressionQuality: 0.8) {
                            ...
                            self?.sink?([
                                "name": "barcode",
                                "data": barcodes.map({ $0.toMap(imageWidth: width, imageHeight: height, scanWindow: self?.scanWindow) }),
                                "image": imageData,
                            ])
                        }
                    })
    ...
    func start(_ call: FlutterMethodCall, _ result: @escaping FlutterResult) {
        ...
        if #available(iOS 13.0, *) {
            ...
            // iOS < 13: use wide-angle to avoid availability issues
            device = AVCaptureDevice.DiscoverySession(
